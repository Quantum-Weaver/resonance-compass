...
`src-tauri/gen/` is **deliberately tracked, not gitignored** — since 2026-08-16 at KP's word
(`.gitignore`, the SANCTUARY-SPECIFIC block: ignoring it had hidden two cross-landings from git).
`npm run tauri android init` still regenerates it wholesale, and every customization below lives
inside `gen/` — so never run `android init` casually (the `child-builds` skill first); if it ever
runs, re-apply the steps below and read `git diff src-tauri/gen` before committing. *(Trued
2026-09-01; this line said "gitignored" from its writing until then.)*
