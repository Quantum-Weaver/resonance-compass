...
|---|---|---|---|---|
| U1 | 2026-08-05 | "bluetooth connectivity not allowing control of player (android auto maybe)" | **Verified: no MediaSession exists anywhere in the app.** The rodio engine plays invisible to Android's media layer, so BT/AVRCP buttons (headphones, car), the lockscreen, and Android Auto have nothing to address. Fix path: an Android MediaSession bridge via the house's own proven pattern (`media_permission.rs` is already a custom Tauri Android plugin — the same extension road). Full Android Auto browsing = MediaBrowserService, a larger second step; MediaSession alone wins BT + headset + lockscreen control. | 🔵 built 2026-08-07 (the Canon lamp) — `MediaSessionPlugin.kt` stands beside its sibling on the same android-extras road (platform `android.media.session` APIs only, no androidx, so gen/'s build.gradle stays untouched): session + transport callbacks → `mediaCommand` events → the player store (the only authority); MediaStyle notification with art gives the lockscreen its handle (POST_NOTIFICATIONS asked once at first play; denied = BT/headset still work, lockscreen controls absent — graceful); `media_session.rs` no-ops Ok on desktop so the store calls unconditionally; state sent on transitions only, never per-tick. Honest line drawn: on Android ≤12 the notification carries no action buttons (needs a manifest media-button receiver — named future work with MediaBrowserService/Android Auto); becoming-noisy pause composes unchanged. Gates: svelte-check 390/0/0 · cargo check clean. The true proof is KP's hands: Android build, BT buttons, lockscreen. |
