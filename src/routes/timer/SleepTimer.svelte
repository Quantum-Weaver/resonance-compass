<script lang="ts">
	...
	onMount(() => {
		endChime = localStorage.getItem(CHIME_KEY) === 'true';
	});

	function setEndChime(v: boolean) {
		endChime = v;
		try {
			localStorage.setItem(CHIME_KEY, String(v));
		} catch {}
	}
...
<div class="timer-page">
	...
	{:else}
		...
		<div class="fade-row chime-row">
			<span>Chime at end</span>
			<button
				class="toggle"
				class:active={endChime}
				onclick={() => setEndChime(!endChime)}
				aria-label="Enable end chime"
				aria-pressed={endChime}
			>
				<span class="dot"></span>
			</button>
		</div>
